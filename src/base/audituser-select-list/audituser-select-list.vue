<template>
  <transition name="slide">
    <div v-if="isShow" class="merchant-select">
      <f7-navbar>
        <f7-nav-left back-link="返回" @back-click="goBack" :sliding="false">
        </f7-nav-left>
        <f7-nav-title>搜索开票员</f7-nav-title>
        <f7-nav-right>
          <f7-link ></f7-link>
        </f7-nav-right>
      </f7-navbar>
      ...
      <div class="scroll-wrapper-box">
        <cube-scroll @pulling-up="searchMore" :options="scrollOptions" :data="list">
          <f7-list media-list class="search-list searchbar-found">
            <f7-list-item @click="onSelectItem(item)"
                          v-for="(item, index) in list" :key="index" :title="item.auditUserName">
            </f7-list-item>
          </f7-list>
        </cube-scroll>
      </div>
