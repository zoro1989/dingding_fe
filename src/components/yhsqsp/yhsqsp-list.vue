<template>
  <transition name="slide">
    <div class="yhsqsp-list">
      ...
      <scroll class="yhsqsp" @scrollToEnd="searchMore" :pullup="pullup" :data="list">
        <f7-list media-list>
          <f7-list-item
            v-for="item in list"
            :key="item.id"
          ...
          >
            <div slot="inner-start">
              ...
              <div class="item-subtitle">
                <div>要货人名称：{{item.require_goods_name}}</div>
                <div>要货人分公司：{{item.require_goods_office}}</div>
                <div>要货人支公司：{{item.require_goods_branch_office}}</div>
              </div>
              <div class="item-text">
