  ...
  <transition name="slide">
    <div class="dhsq-list">
      <f7-navbar>
        ...
        <!--</f7-nav-left>-->
        <f7-nav-title>调货申请列表</f7-nav-title>
        <f7-nav-right>
          <f7-link @click="addApply">新增</f7-link>
        </f7-nav-right>
      </f7-navbar>
      <scroll class="dhsq" @scrollToEnd="searchMore" :pullup="pullup" :data="list">
        <f7-list media-list>
          <f7-list-item
            swipeout
            v-for="item in list"
          ...
          >
            <div slot="inner-start">
              ...
              <div class="item-text">调货人名称：{{item.arrangeGoodsName}}</div>
              <div class="item-text">调货人分公司：{{item.arrangeGoodsOffice}}</div>
              <div class="item-text">调货人支公司：{{item.arrangeGoodsBranchOffice}}</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      initData() {
        fetch('get', api.arrangeGoodsInfo, {page: this.pageNo, limit: this.pageSize}, this).then((res) => {
          console.log(res)
          this.list = res.data
      ...
      onDelete(id) {
        ...
        app.dialog.confirm('确定要删除吗?', '提示', function () {
          ...
          fetch('delete', api.arrangeGoodsInfo + id, {}, this).then((res) => {
            console.log(res)
          })
        })
      },
